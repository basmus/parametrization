...
[MemoryDiagnoser]
public class NamedParametrizerBenchmarks
{
    private NamedParametrizer _parametrizer;
    ...
    public void Setup()
    {
        var builder = new TestNamedPlaceholderBuilder();
        _parametrizer = new NamedParametrizer(builder);
    }
    ...
    private static Parameter[] GenerateParameters(int count)
    {
        ...
        var namedCount = count / 2;

        for (var i = 0; i < namedCount; i++)
        {
            parameters[i] = new Parameter($"param{i}", i);
        }

        for (var i = namedCount; i < count; i++)
        {
